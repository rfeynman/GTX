/* map1D_E.c - Reads a 1D table of axial Ez samples and extrapolates these
 *             to a cylindrical symmetric field map for the electric field
 */
...
void map1D_E_init(gptinit *init)
{
  ...
  if( gptgetargnum(init)!=4 )
    gpterror( "Syntax: %s(ECS,mapfile.gdf,z,Ez,Efac)\n", gptgetname(init) ) ;
