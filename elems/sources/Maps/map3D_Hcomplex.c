/* map3D_Hcomplex.c - 3D rectangular field map for a complex magnetic field */
...
void map3D_Hcomplex_init(gptinit *init)
{
  ...
  int numc ;
  const char *fxrename,*fyrename,*fzrename ;
  const char *fximname,*fyimname,*fzimname ;
  ...
  if( numc!=13 )
    gpterror( "Syntax: %s(ECS,mapfile.gdf,x,y,z,Hxre,Hyre,Hzre,Hxim,Hyim,Hzim,ffac,phi,w)\n", gptgetname(init) ) ;
 ...
 */
  ...
  BX = gpt_mu0*(fxre*cosphase - fxim*sinphase);
  BY = gpt_mu0*(fyre*cosphase - fyim*sinphase) ;
  BZ = gpt_mu0*(fzre*cosphase - fzim*sinphase) ;
