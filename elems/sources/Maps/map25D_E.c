/* map25D_E.c - 2.5D rectangular, cylindrical symmetric field map 
...
void map25D_E_init(gptinit *init)
{
  ...
  int numc ;
  const char *frname,*fphiname,*fzname ;
  ...
  if( numc!=7 )
    gpterror( "Syntax: %s(ECS,mapfile.gdf,r,z,Er,Ephi,Ez,Efac)\n", gptgetname(init) ) ;
  ...
  fphiname = gptgetargstring(init,5) ;
  fzname   = gptgetargstring(init,6) ;
 ...
 */
  ...
  Ephi = f1*info->fphi[N1] + f2*info->fphi[N2] + f3*info->fphi[N3] + f4*info->fphi[N4] ;
  EZ   = f1*info->fz[N1]   + f2*info->fz[N2]   + f3*info->fz[N3]   + f4*info->fz[N4] ;

  gptrphi2carth(Er,Ephi,X,Y,&EX,&EY) ;
