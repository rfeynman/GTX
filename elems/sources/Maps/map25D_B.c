/* map25D_B.c - 2.5D rectangular, cylindrical symmetric field map 
...
void map25D_B_init(gptinit *init)
{
  ...
  if( numc!=7 )
    gpterror( "Syntax: %s(ECS,mapfile.gdf,r,z,Br,Bphi,Bz,Bfac)\n", gptgetname(init) ) ;
 ...
 */
  ...
  Br   = f1*info->fr[N1]   + f2*info->fr[N2]   + f3*info->fr[N3]   + f4*info->fr[N4] ;
