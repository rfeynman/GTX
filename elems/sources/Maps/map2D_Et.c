/* map2D_Et.c - 2D electrostatic field-map with linear increasing amplitude as function of time */
...
void map2D_Et_init(gptinit *init)
{
  ...
  int numc ;
  const char *frname,*fzname ;
  const char *rname,*zname ;
  double ffac ;
  ...
  if( numc!=7 )
    gpterror( "Syntax: %s(ECS,mapfile.gdf,r,z,Er,Ez,fac,tau)\n", gptgetname(init) ) ;
  ...
  /* Store magnetic field, loops can be more efficient */
  for(j=0 ; j<info->numz ; j++)
  {
    double integREz = 0.0 ;

    for(i=0 ; i<info->numr ; i++)
    {
      N=j*info->numr+i ;
      ...
      integREz += info->fz[N] * (i-0.5)*info->rdelta ;
      info->fphi[N] = integREz/(i*gpt_c*gpt_c*info->tau) ;
    }
  }
 ...
 * if(ft<0 || ft>1+16*DBL_EPSILON ) gptrror( "r fraction is %.16f at r=%f\n", ft, R ) ;
...
static void swapmap2D(struct map2d_info *info, int i, int j)
{
  ...
  /* Note: Bphi is calculated, no need to swap */
