/* setcharR.c - Set the total charge of a set 
 *              for the space-charge model based on circles
 */
...
void setcharge2Dcircle_init(gptinit *init)
{
  gptparset *set ;
  gptinitpar *par ;
  const char *name ;
  ...
  for( i=0 ; i<len ; i++ )
  {
    ...
    if( par[i].q == 0 ) zero++ ;
    if( par[i].q > 0 ) pos++ ;
    r = sqrt(par[i].Wr[0]*par[i].Wr[0]+par[i].Wr[1]*par[i].Wr[1]) ;
    sum += r*par[i].q ;
  }
  if( sum==0.0 )
    gpterror( "Invalid distribution of elementary charges\n" ) ;
