...
// Defer to point-to-point method for small N
  if( numpar<TREEMINPAR )
  {
    sc3Dp2p(t,x,p,info->zmin,info->zmax) ;
    return ;
  }
...
// Get bunch center of particles (not corrected for n), and rmacro
  ...
  for(int i=0 ; i<numpar ; i++) if( pars[i].alive )
  {
    ...
    if( pars[i].r2 != eps2 )
		gpterror( "Particle radii must be identical. Use 'setrmacrodist(\"%s\",\"u\",rmacro,0).", pars[i].set->name ) ;
  }
...
#pragma omp parallel for schedule(guided,mc_minpar_elems) num_threads(std::min(1+numpar/mc_minpar_elems,mc_threads_elems)) if(mc_threads_elems!=1 && numpar>mc_minpar_elems)
  for(int i=0 ; i<numpar ; i++)
    if( pars[i].alive )
  {
...
static int restframeparams(struct sctree_info *info)
{
  ...
  /* Post check for valid rest-frame". If not, give up. */
  if(!(info->Go>=1))
	  gpterror("No valid rest frame can be found.") ;
