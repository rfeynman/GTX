...
#pragma omp parallel for schedule(static) num_threads(gptCPUs) if(gptCPUs!=1 && numpar > 16*gptCPUs)
      ...
      for(int j=0 ; j<numpar ; j++)
  {
    ...
    double Bcirc = parj->GBr[2]/parj->G ;
    double Gcirc = 1/sqrt(1-Bcirc*Bcirc) ;
    double Rcirc = sqrt(parj->Wr[0]*parj->Wr[0]+parj->Wr[1]*parj->Wr[1]) ;
    ...
    double lambda = parj->q*parj->n/(2*gpt_pi*Rcirc) ;
    double E[3] ;
