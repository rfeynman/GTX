...
void sectormagnet_init(gptinit *init)
{
  ...
  if( argc!=4 && argc!=9)
    gpterror( "Syntax: %s(fromaxis, toaxis, arclength, angle, Bfield, [phi_in, phi_out, dl, b1, b2])\n", gptgetname(init) ) ;
...
/* Fill coefficients for local use. */
  length = gptgetargdouble(init,3) ;
  angle  = gptgetargdouble(init,3) ;
  if(fabs(angle) < 1e-6)
    gpterror( "angle is less than 1e-6 rad, does not make sense\n");
