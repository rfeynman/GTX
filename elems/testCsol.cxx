...
void csol_init()
{
  ...
  /* Initialize GDF */
//   printf("numc %d\n", numc);
  filename = "/home/jorg/LowEnergyCooling/Dima/realSolenoidField/Fe_map_Jan_2016.bin";
//   printf("file %s\n", filename);
 	double solfac  = 0. ;
 	double hdipfac = 1. ;
 	double vdipfac = 0. ;
...
//   printf("factor %e\n", ffac);
  ...
  if(64 != nb)
  {
	  fprintf(stderr, "first read only %ld bytes, must be 32\n",  nb);
	  exit(-1);
  }
  if(10071951 != iheader[0])
  {
	  fprintf(stderr, "wrong magic number in csol file\n");
	  exit(-1);
  }

  	info = (struct csol_info *) malloc( sizeof(struct csol_info ) ) ;
	info->nsr    = iheader[2] ; 
	info->nsz    = iheader[3] ; 
	info->ndx    = iheader[4] ; 
	info->ndy    = iheader[5] ; 
  ...
  float * allFields = info->allFields = ((float *)malloc( nbs )) ;
  nb = read(fp, info->allFields, nbs );
  close(fp);
  if(nbs != nb) fprintf(stderr,  "second  read only %ld bytes, must be %ld\n", nb, nbs);



  int grps =0;
  int nfl = (2*spoints +6*dpoints +6*qpoints);  /* number of bytes for all 3 field components */
  float old = allFields[0];
  for(int i=0; i< nfl; i++)
  {
	  if(allFields[i] != old)
	  {
...
static int csol_sim(double X, double Y, double Z)
{
  int i,j,k ;
...
main(int argc, char ** argv)
{
	...
	while(1)
	{
		...
		csol_sim(X*0.001, Y*0.001, Z*0.001);
	}
