/* timesetfile.c - Add a group in a GDF file to a particle set */
...
void timesetfile_init(gptinit *init)
{
  char groupname[256] ;
  ...
  if( gptgetargnum(init)!=3 )
    gpterror( "Syntax: %s(set, gpt_outout_filename, time)\n", gptgetname(init) ) ;

  const char * setname     = gptgetargstring(init,1) ;
  const char * filename = gptgetargstring(init,2) ;
//   const char * axisname = gptgetargstring(init,3) ;
  const char * axisname = "wcs";
  double time     = gptgetargdouble(init,3) ;

  gptparset *set = gptgetparset( setname ) ;
  struct axis *paxis = getaxis( axisname ) ;
  ...
  struct gdfdata * ds = gm.ds.childs;
  struct gdfdata *dst =0;
  double tdiff=1e33;
  ...
  while(ds)
  {
	const char * name = (char *) (ds->oh.name);
	if( ! strcmp("time", name) )
	{
  		double  tim = *((double * ) ds->buf);
		double ttdiff =fabs(time-tim);
		if(ttdiff < tdiff)
		{
			dst = ds;
			tdiff = ttdiff;
  ...
  if(dst)
  {
	  double  tim = *((double * ) dst->buf);
	  printf("closest time block to %e found at %e\n", time, tim);
	  ds = dst-> childs;
  }
  else
  {
	  printf("no time for old men\n");
   	  exit(1);
  }
