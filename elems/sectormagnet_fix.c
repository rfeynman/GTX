 ...
 *       : Extra iteration to increase the precision of the intersection point
 */

/* With special thanks to Bruno Muratory from Daresbury Laboratory
 * for providing the fringe field expressions
 */
...
#define FUZZFACTOR (2048*DBL_EPSILON)
...
static int sectormagnet_ccsflip(gptpar *par,double t,struct sectormagnet_info *info) ;

static void inittoaxis(gptinit *init,struct axis *axis) ;
static void gptinversetransform(gpttransform *u, const gpttransform *t) ;

void sectormagnet_fix_init(gptinit *init)
{
  ...
  const char *fromaxisname, *toaxisname ;
...
/* Newton Raphson iteration to increase precision */
  dpq = INP(d,P)-INP(d,Q) ;
  epq = INP(e,P)-INP(e,Q) ;
  t = (de*epq - dpq)/divf ;
  u = (epq - de*dpq)/divf ;
  for(i=0 ; i<3 ; i++) P[i] += t*d[i] ;
  for(i=0 ; i<3 ; i++) Q[i] += u*e[i] ;
...
/* Add to fromaxis */
  ...
  createidentitytransform(&init->e) ;
  init->e.o[2] = t ;
  init->paxis = fromaxis ;
  ...
  gptaddEBelement( init, sectormagnet_ccsflip, NULL, GPTELEM_GLOBAL, info ) ;
  ...
  else
  {
    ...
    inittoaxis(init,toaxis) ;
    gptaddEBelement( init, sectormagnet_sim, NULL, GPTELEM_GLOBAL, info ) ;
  }
...
static int sectormagnet_sim(gptpar *par,double t,struct sectormagnet_info *info)
{
  ...
  if( fabs(d*info->b1) > NGAP )
  {
    double B = (d<0 ? 0.5 : -0.5) * info->B ;
    BX = B*info->f[0] ;
    BY = B*info->f[1] ;
    BZ = B*info->f[2] ;
    return 1 ;    
  ...
  Bf = Bn*(1+exp(f)*cos(h)) - 0.5*info->B ;
  Bm = Bn*exp(f)*sin(h) ;
...
// Hard-edge version without fringes
static int sectormagnet_simple_sim(gptpar *par,double t,struct sectormagnet_info *info)
{
...
// Moves a particle to the other axis when crossing the midplane
static int sectormagnet_ccsflip(gptpar *par,double t,struct sectormagnet_info *info)
{
  if( gptVECINP(par->r, info->n) < 0 )
    par->newaxis = info->otheraxis ;

  return 0 ;
}

// Rewrite an element to a new coordinate system
static void inittoaxis(gptinit *init,struct axis *axis)
{
  gpttransform inverseaxis, wcstransform ;

  gptconcattransform(&wcstransform,&init->paxis->a,&init->e) ;

  gptinversetransform(&inverseaxis,&axis->a) ;
  gptconcattransform(&init->e,&inverseaxis,&wcstransform) ;

  init->paxis = axis ;
}

// Store the inverse transform of t into u
static void gptinversetransform(gpttransform *u, const gpttransform *t)
{
  // Do the logic
  for(int i=0 ; i<3 ; i++)
