...
void map3D3Sym_B_init(gptinit *init)
{
...
static int map3d_sim(gptpar *par, double t, struct map3d_info *info)
{
  ...
  if( ZZ<info->zmin || ZZ>info->zmax ) return( 0 ) ;
  if( YY<info->ymin || YY>info->ymax ) return( 0 ) ;
  if( XX<info->xmin || XX>info->xmax ) return( 0 ) ;

  /* Calculate master index and offset fractions */
  i = (int)((XX-info->xmin)/info->xdelta) ;
  j = (int)((YY-info->ymin)/info->ydelta) ;
  k = (int)((ZZ-info->zmin)/info->zdelta) ;
  N = (k*info->numy+j)*info->numx+i ;
  ft= (XX-info->xmin-i*info->xdelta)/info->xdelta ;
  fu= (YY-info->ymin-j*info->ydelta)/info->ydelta ;
  fv= (ZZ-info->zmin-k*info->zdelta)/info->zdelta ;
  gt = 1-ft ;
  gu = 1-fu ;
  gv = 1-fv ;
